"""Run Task 2's complete 144-configuration, MLflow-tracked experiment.
...
ASSIGNMENT2_DIR = TASK2_DIR.parent
PROJECT_ROOT = ASSIGNMENT2_DIR.parent
...
def clean_data(frame):
    ...
    # A1 deletes the small group of rows missing any of these four fields.
...
def main():
    ...
    tracking_db = TASK2_DIR / "mlflow_task2_a1.db"
    ...
    results.to_csv(TASK2_DIR / "task2_experiment_results.csv", index=False)
    best = results.iloc[0].to_dict()
    bundle = fit_best_model(X_train, y_train, X_test, y_test, best)

    models_dir = TASK2_DIR / "models"
    models_dir.mkdir(exist_ok=True)
    joblib.dump(bundle, models_dir / "best_task2_model.joblib")
    summary = {"best_config": best, "test_metrics": bundle["test_metrics"]}
    (TASK2_DIR / "task2_best_model_summary.json").write_text(
